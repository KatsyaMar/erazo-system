...
def _notificar_usuario(cursor, usuario, asunto, mensaje):
    """
    Guarda la notificación interna y la notificación por correo del paciente.
    """
    _guardar_notificacion(cursor, usuario['id_usuario'], mensaje, 'CORREO')
    _guardar_notificacion(cursor, usuario['id_usuario'], mensaje, 'INTERNA')

    return _enviar_correo_notificacion(
        usuario['correo'],
